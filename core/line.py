...
@ti.data_oriented
class LineRaster:
    def __init__(self, engine, maxwires=MAX, linewidth=2,
                  linecolor=(.9, 0, .5), clipping=False, **extra_options):
        self.engine = engine
        self.res = self.engine.res
        self.maxlines = maxwires
    ...
    @ti.kernel
    def set_object(self, line: ti.template()):
        # if hasattr(mesh, 'get_nfaces'):
        #     mesh.pre_compute()
        ...
        line.pre_compute()
        self.nwires[None] = line.get_nlines() - 1
        for i in range(self.nwires[None]):
            verts = line.get_line_verts(i)
            for k in ti.static(range(2)):
                self.verts[i, k] = verts[k]
        # else:
        #     self.nwires[None] = mesh.get_nlines
        #     verts = mesh.get_line_verts()
        #     for k in ti.static(range(2)):
        #         self.verts[k] = verts[k]
